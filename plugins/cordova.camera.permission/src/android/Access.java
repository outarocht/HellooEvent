 ...
 */
public class Access extends CordovaPlugin{
    ...
    static {
        ...
        Access.addBiDirMapEntry(_permissionsMap, "CAMERA", Manifest.permission.CAMERA);
    ...
    protected JSONObject _getPermissionsAuthorizationStatus(String[] permissions) throws Exception{
        ...
        for(int i=0; i<permissions.length; i++){
            ...
            if(granted){
                statuses.put(permission, Access.STATUS_GRANTED);
            }else{
                ...
                if(!showRationale){
                    if(isPermissionRequested(permission)){
                        statuses.put(permission, Access.STATUS_DENIED_ALWAYS);
                    }else{
                        statuses.put(permission, Access.STATUS_NOT_REQUESTED);
                    }
                }else{
                    statuses.put(permission, Access.STATUS_DENIED);
                }
            }
        }
        return statuses;
    ...
    protected void _requestRuntimePermissions(JSONArray permissions, int requestId) throws Exception{
        ...
        for(int i = 0; i<currentPermissionsStatuses.names().length(); i++){
            ...
            boolean granted = currentPermissionsStatuses.getString(permission) == Access.STATUS_GRANTED;
            if(granted){
                ...
                requestStatuses.put(permission, Access.STATUS_GRANTED);
    ...
    public void executePluginJavascript(final String jsString){
        executeGlobalJavascript("cordova.camera.access." + jsString);
    }
