...
public class OpenTokCustomVideoRenderer extends BaseVideoRenderer {
    ...
    public JSONArray illustrationContext;
    public byte[] illustrationY;
    public byte[] illustrationU;
    public byte[] illustrationV;
    ...
    public void updateIllustration(JSONArray illustrationData) {
        if(illustrationData == null) {
            this.illustrationContext = null;
            this.illustrationY = null;
            this.illustrationU = null;
            this.illustrationV = null;
        }
        else {
            this.illustrationContext = illustrationData;
        }
        
        try {
            Log.i("OTPlugin - updateIllustration() length", Integer.toString(illustrationData.toString().length()));

            //Retrieve data from the given JSON Array
            //JSONArray imageRGB = illustrationData.getJSONArray(0);
            JSONArray imageRGB = new JSONArray(illustrationData.getString(0));
            int width = illustrationData.getInt(1);
            int height = illustrationData.getInt(2);

            //Make buffer arrays for the YUV values that we can shell out to while converting
            byte[] yArr = new byte[width * height];
            byte[] uArr = new byte[width * height];
            byte[] vArr = new byte[width * height];

            //Loop through the image's RGB values and convert them to YUV
            for(int i = 0; i < imageRGB.length(); i += 4) {
                int red = imageRGB.getInt(i);
                int green = imageRGB.getInt(i + 1);
                int blue = imageRGB.getInt(i + 2);

                yArr[i/4] = (byte) (Math.round(0.299f * red + 0.587f * green + 0.114f * blue) - 128);
                uArr[i/4] = (byte) (Math.round(-0.147f * red - 0.289f * green + 0.436f * blue) - 128);
                vArr[i/4] = (byte) (Math.round(0.615f * red - 0.515f * green - 0.100f * blue) - 128);
            }

            //Update the public arrays
            this.illustrationY = yArr;
            this.illustrationU = uArr;
            this.illustrationV = vArr;
        }
        catch(Exception e) {
            Log.i("OTPlugin - updateIllustration()", e.toString());
        }
    ...
    public void onFrame(Frame frame) {
        ...
        if(this.illustrationY != null && this.illustrationU != null && this.illustrationV != null) {
            ...
            //Loop through every U value
            for (int i = startU; i < 2 * startU; i++) {
                byte u = this.illustrationU[i];
                imageBuffer.put(i, u);
                // if(u != 0) {
            ...
            //Loop through every V value
            for (int i = startU * 2; i < 3 * startU; i++) {
                byte v = this.illustrationV[i];
                imageBuffer.put(i, v);
                // if(v != 0) {
